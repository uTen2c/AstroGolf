    ...
    height(height)
{
    ...
    ps_handle_ = LoadPixelShader("shader/box.pso");
    spdlog::info("ps_handle_: {}", ps_handle_);
}
...
void BoxComponent::Draw(DrawStack* stack)
{
    ...
    const auto x1 = vec.x + start.x;
    const auto x2 = vec.x + end.x;
    const auto y1 = vec.y + start.y;
    const auto y2 = vec.y + end.y;
