...
void CommonPlanetComponent::UpdateSatellite(float deltaTime)
{
    ...
    const auto offset = Vec2(rotation_distance_, 0).Rotate(rotation_angle_);
    transform.translate = origin_world_pos_ + offset;
}

void CommonPlanetComponent::UpdateInEditor(float deltaTime)
{
    if (rotationOriginDragging)
    {
        const auto mousePos = Game::Device().MousePos();
        const auto zoom = world->GetCamera().zoom;
        const auto moved = mousePos - rotationOriginDragStartMousePos;
        rotationOriginOffset = rotationOriginDragStartOffset + moved / zoom;
    }
}
...
void CommonPlanetComponent::Draw(DrawStack* stack)
{
    ...
    if (Game::debugEnabled || Game::instance->GetWorld().GetType() == WorldType::Editor)
    {
        ...
        if (isSatellite)
        {
            ...
            const auto originColor = rotationOriginDragging ? GetColor(63, 63, 255) : GetColor(255, 255, 255);
            ...
            if (Game::Device().LeftClicked())
            {
                ...
                {
                    rotationOriginDragging = true;
                    rotationOriginDragStartMousePos = mousePos;
                    rotationOriginDragStartOffset = rotationOriginOffset;
                    if (world)
                    {
            ...
            if (Game::Device().LeftReleased() && rotationOriginDragging)
            {
                rotationOriginDragging = false;
                if (world)
                {
